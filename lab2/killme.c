...
struct Options get_opts(int count, char* args[]) {
    ...
    //while(&args_temp[1][c] != NULL) {
        ...
        // my thinking is that the numbers are not led by a - so as long as the string is not NULL it should enter this loop
            ...
            args_temp[1][0] == '0' ? strncat(opts.process_id, "0", 127) : 0; 
            args_temp[1][0] == '1' ? strncat(opts.process_id, "1", 127) : 0; 
            args_temp[1][0] == '2' ? strncat(opts.process_id, "2", 127) : 0; 
            args_temp[1][0] == '3' ? strncat(opts.process_id, "3", 127) : 0; 
            args_temp[1][0] == '4' ? strncat(opts.process_id, "4", 127) : 0; 
            args_temp[1][0] == '5' ? strncat(opts.process_id, "5", 127) : 0; 
            args_temp[1][0] == '6' ? strncat(opts.process_id, "6", 127) : 0; 
            args_temp[1][0] == '7' ? strncat(opts.process_id, "7", 127) : 0; 
            args_temp[1][0] == '8' ? strncat(opts.process_id, "8", 127) : 0; 
            args_temp[1][0] == '9' ? strncat(opts.process_id, "9", 127) : 0; 
            //break;
        }
        //c++;
    //}
